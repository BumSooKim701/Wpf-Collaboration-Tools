...
public class ConnectionPool : IDisposable
{
    ...
    private static readonly object _instanceLock = new object(); //인스턴스 처리용 락
    private readonly object _poolLock = new object(); //풀 커넥션 처리용 락
    ...
    private int _currentConnectionCount = 0; //현재 총 커넥션 수
    ...
    {
        for (int i = 0; i < MIN_POOL_SIZE; i++, _currentConnectionCount++)
        {
    ...
    public void Dispose()
    {
        if (!_disposed)
        {
            lock (_poolLock)
            {
                ...
                {
                    _connectionPool.Dequeue()?.Dispose();
                }
